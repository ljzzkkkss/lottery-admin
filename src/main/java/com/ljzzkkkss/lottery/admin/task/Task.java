...
//    @Scheduled(cron = "*/1 * * * * ?")
    public void getOdd(){
        ...
        String page = restTemplate.getForEntity("https://evs.500.com/esinfo/lotinfo/lot_info?lotid=46&lottype=huntou&page=1&pagesize=100&webviewsource=touch&platform=touch",String.class).getBody();
        JSONObject pageJSON = JSONObject.parseObject(page);
        JSONArray matchList = pageJSON.getJSONArray("data");
        for(int i = 0; i < matchList.size();i++){
            try{
                JSONObject matchJSON = matchList.getJSONObject(i).getJSONObject("mdata");
                JSONObject oddJSON = matchList.getJSONObject(i).getJSONObject("pdata");
                ...
                String oddData = restTemplate.getForEntity("https://evs.500.com/esinfo/lotinfo/lot_info?lotid=46&lottype=huntou_more&matchesid=" + matchId + "&webviewsource=touch&platform=touch",String.class).getBody();
